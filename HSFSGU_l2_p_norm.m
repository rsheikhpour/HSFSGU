%% HSFSGU(?2,1/2-norm) algorithm
% This sript is the code for the HSFSGU(?2,1/2-norm) algorithm for
...
function [Z]=HSFSGU_l2_p_norm(X_label,X_unlabel,Y_label,H,beta,lambda)
%% Input
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
while t < 100
    ...
    S_td = X_tr*X_tr' + (4*lambda*D);
    N =((S_td)^(-1/2))*X_tr*F;
    [U,~,V] = svd(N, 0);
    M = U*V';
    Z = S_td^(-1/2)*M; 
    zi=(sum(Z.^2,2)+eps).^(1/4);
    D = diag(1./(4*(zi.^3)));
    ...
    obj(iter) =(norm((X_train'*Z - alpha*F), 'fro'))^2  + beta*trace(F'*LP*F) + lambda*sum((sum(Z.*Z,2)+eps).^(1/4));
